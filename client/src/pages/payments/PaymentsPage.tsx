...
//   const load = () => {
//     api.get('/api/payments').then(({ data }) => setPayments(data.data || []));
//     if (user?.role === 'society_admin') {
//       api.get('/api/payments/summary').then(({ data }) => setSummary(data.data));
//     }
//   };
...
//     const now = new Date();
//     await api.post('/api/payments', { month: now.getMonth() + 1, year: now.getFullYear() });
//     toast.success('Invoices generated');
//     load();
//   };

//   const pay = async (id: string) => {
//     await api.patch(`/api/payments/${id}/pay`, { paymentMethod: 'online' });
