...
exports.getUsers = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const filter = { ...getSocietyFilter(req), isActive: true };
    if (req.query.role)
    ...
    const [users, total] = await Promise.all([
        models_1.User.find(filter).select('-password').sort({ createdAt: -1 }).skip(skip).limit(limit),
...
exports.addFamilyMember = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
...
exports.removeFamilyMember = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.role === 'resident' ? req.user._id : req.params.id;
    const user = await models_1.User.findById(userId);
    if (!user)
        throw new ApiError_1.ApiError(404, 'User not found');
...
exports.getMaintenanceStaff = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
